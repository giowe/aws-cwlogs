...
function _getAllLogGroups(cloudwatchlogs, cb, nextToken, out = []) {
  cloudwatchlogs.describeLogGroups({ nextToken }, (err, data) => {
    ...
    if (nextToken) {
      _getAllLogGroups(cloudwatchlogs, cb, nextToken, [...logGroups, ...out])
    } else {
      cb(null, [...logGroups, ...out])
    }
  })
}
...
function _getLogGroups(options, cb) {
  ...
  _getAllLogGroups(cloudwatchlogs, (err, data) => {
    if (err) {
      return console.log(clc.red("Error:\n"), err)
    }
    ...
    if (logGroupNames.length === 0) {
      return cb(`No log groups found on AWS ClodWatch Logs in ${region} region`)
    }
...
function _getLogStreams(options, cb) {
  ...
  cloudwatchlogs.describeLogStreams(params, (err, data) => {
    ...
    if (logStreamNames.length === 1) {
      return console.log(clc.red(`No log streams found in ${logGroupName}`))
    }
...
function _startLogging (options) {
  if (options.logStreamName === _latestLabel) {
    options.logStreamName = null
  }
...
const commands = {
  ...
  help: (command) => {
    ...
    if (commandInfo) {
      return console.log(commandInfo)
    }
  ...
  configure: () => {
    ...
    prompt([
      ...
      if (_saveConfig(config)) {
        console.log(`${clc.green("Successfully")} saved configurations`)
      }
    })
      .catch(console.log)
  }
